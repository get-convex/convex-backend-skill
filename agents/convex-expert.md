...
- **Copy this pattern instead of re-deriving it per function.** The three rules above (identity-from-arg, missing ownership check, PII-by-argument) are the single largest real-defect cluster measured against generated Convex backends — 44 of 214 confirmed defects in one 30-app corpus. Two small helpers close all three at once — **but only once the foundation check above passes**:
